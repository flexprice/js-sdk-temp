...
export function subscriptionsCancelSubscriptionSchedule(
  ...
  Result<
    shared.DtoCancelScheduleResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CancelSubscriptionScheduleRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.CancelScheduleRequest"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.DtoCancelScheduleResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
