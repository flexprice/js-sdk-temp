 ...
 */
export function webhooksPostWebhooksMoyasarTenantIdEnvironmentId(
...
> {
  const input: operations.PostWebhooksMoyasarTenantIdEnvironmentIdRequest = {
    tenantId: tenantId,
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.PostWebhooksMoyasarTenantIdEnvironmentIdRequest$outboundSchema
        .parse(value),
  ...
  const path = pathToFunc("/webhooks/moyasar/{tenant_id}/{environment_id}")(
    pathParams,
  );

  const headers = new Headers(compactMap({
    Accept: "application/json",
    "X-Moyasar-Signature": encodeSimple(
      "X-Moyasar-Signature",
      payload["X-Moyasar-Signature"],
