...
export function entitlementsCreateEntitlementsBulk(
  ...
  Result<
    operations.CreateEntitlementsBulkResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      shared.DtoCreateBulkEntitlementRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/entitlements/bulk")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateEntitlementsBulkResponse$inboundSchema),
    M.json(400, operations.CreateEntitlementsBulkResponse$inboundSchema),
    M.json(500, operations.CreateEntitlementsBulkResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
