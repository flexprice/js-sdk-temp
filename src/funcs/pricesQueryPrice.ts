...
> {
  const parsed = safeParse(
    request,
    (value) => shared.TypesPriceFilter$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.QueryPriceResponse$inboundSchema),
    M.json(400, operations.QueryPriceResponse$inboundSchema),
    M.json(500, operations.QueryPriceResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
