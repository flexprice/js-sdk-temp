 ...
 */
export function customerPortalGetPortalExternalId(
  client: FlexPriceCore,
  externalId: string,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.DtoPortalSessionResponse,
...
> {
  const input: operations.GetPortalExternalIdRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.GetPortalExternalIdRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/portal/{external_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.DtoPortalSessionResponse$inboundSchema),
