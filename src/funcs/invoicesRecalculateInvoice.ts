...
export function invoicesRecalculateInvoice(
  ...
  Result<
    operations.RecalculateInvoiceResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.RecalculateInvoiceRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.RecalculateInvoiceResponse$inboundSchema),
    M.json([400, 404], operations.RecalculateInvoiceResponse$inboundSchema),
    M.json(500, operations.RecalculateInvoiceResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
