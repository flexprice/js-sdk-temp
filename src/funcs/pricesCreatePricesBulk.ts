...
export function pricesCreatePricesBulk(
  ...
  Result<
    operations.CreatePricesBulkResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) => shared.DtoCreateBulkPriceRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreatePricesBulkResponse$inboundSchema),
    M.json(400, operations.CreatePricesBulkResponse$inboundSchema),
    M.json(500, operations.CreatePricesBulkResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
