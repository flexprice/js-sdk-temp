...
> {
  const parsed = safeParse(
    request,
    (value) => models.UpdateSubscriptionRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.UpdateSubscriptionRequest"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdateSubscriptionResponse$inboundSchema),
    M.json(400, models.UpdateSubscriptionResponse$inboundSchema),
    M.json(500, models.UpdateSubscriptionResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
