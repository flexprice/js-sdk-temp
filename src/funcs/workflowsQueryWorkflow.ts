...
export function workflowsQueryWorkflow(
  ...
  Result<
    operations.QueryWorkflowResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) => shared.TypesWorkflowExecutionFilter$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/workflows/search")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.QueryWorkflowResponse$inboundSchema),
    M.json(400, operations.QueryWorkflowResponse$inboundSchema),
    M.json(500, operations.QueryWorkflowResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
