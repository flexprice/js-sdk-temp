...
export function paymentsDeletePayment(
  ...
  Result<
    operations.DeletePaymentResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DeletePaymentRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DeletePaymentResponse$inboundSchema),
    M.json([400, 404], operations.DeletePaymentResponse$inboundSchema),
    M.json(500, operations.DeletePaymentResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
