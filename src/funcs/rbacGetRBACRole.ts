...
export function rbacGetRBACRole(
  ...
  Result<
    operations.GetRbacRoleResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetRbacRoleRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/rbac/roles/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetRbacRoleResponse$inboundSchema),
    M.json(404, operations.GetRbacRoleResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
