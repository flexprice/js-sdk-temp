...
export function walletsGetCustomerWallets(
  ...
  Result<
    operations.GetCustomerWalletsResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetCustomerWalletsRequest$outboundSchema.optional().parse(
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetCustomerWalletsResponse$inboundSchema),
    M.json([400, 404], operations.GetCustomerWalletsResponse$inboundSchema),
    M.json(500, operations.GetCustomerWalletsResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
