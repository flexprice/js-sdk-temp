...
export function webhooksHandleStripeWebhook(
  ...
  Result<
    operations.HandleStripeWebhookResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.HandleStripeWebhookRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.HandleStripeWebhookResponse$inboundSchema),
    M.json(400, operations.HandleStripeWebhookResponse$inboundSchema),
    M.json(500, operations.HandleStripeWebhookResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
