...
export function plansSyncPlanPrices(
  ...
  Result<
    operations.SyncPlanPricesResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SyncPlanPricesRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SyncPlanPricesResponse$inboundSchema),
    M.json([400, 404, 422], operations.SyncPlanPricesResponse$inboundSchema),
    M.json(500, operations.SyncPlanPricesResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
