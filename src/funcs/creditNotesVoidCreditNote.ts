...
> {
  const parsed = safeParse(
    request,
    (value) => models.VoidCreditNoteRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/creditnotes/{id}/void")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.VoidCreditNoteResponse$inboundSchema),
    M.json([400, 401, 403, 404], models.VoidCreditNoteResponse$inboundSchema),
    M.json(500, models.VoidCreditNoteResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
