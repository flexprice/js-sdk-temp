...
> {
  const parsed = safeParse(
    request,
    (value) => models.UpdateEnvironmentRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.UpdateEnvironmentRequest"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdateEnvironmentResponse$inboundSchema),
    M.json([400, 404], models.UpdateEnvironmentResponse$inboundSchema),
    M.json(500, models.UpdateEnvironmentResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
