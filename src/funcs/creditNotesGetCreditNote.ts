 ...
 */
export function creditNotesGetCreditNote(
...
> {
  const parsed = safeParse(
    request,
    (value) => models.GetCreditNoteRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/creditnotes/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.GetCreditNoteResponse$inboundSchema),
    M.json([400, 404], models.GetCreditNoteResponse$inboundSchema),
    M.json(500, models.GetCreditNoteResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
