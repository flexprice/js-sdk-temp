...
export function pricesGetPriceByLookupKey(
  ...
  Result<
    operations.GetPriceByLookupKeyResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetPriceByLookupKeyRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetPriceByLookupKeyResponse$inboundSchema),
    M.json(400, operations.GetPriceByLookupKeyResponse$inboundSchema),
    M.json(500, operations.GetPriceByLookupKeyResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
