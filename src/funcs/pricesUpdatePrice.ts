...
> {
  const parsed = safeParse(
    request,
    (value) => models.UpdatePriceRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.UpdatePriceRequest"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdatePriceResponse$inboundSchema),
    M.json(400, models.UpdatePriceResponse$inboundSchema),
    M.json(500, models.UpdatePriceResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
