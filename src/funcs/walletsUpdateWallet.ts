...
> {
  const parsed = safeParse(
    request,
    (value) => models.UpdateWalletRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.UpdateWalletRequest"], {
    explode: true,
  });
  ...
  const path = pathToFunc("/wallets/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdateWalletResponse$inboundSchema),
    M.json([400, 404], models.UpdateWalletResponse$inboundSchema),
    M.json(500, models.UpdateWalletResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
