...
> {
  const parsed = safeParse(
    request,
    (value) => models.UpdatePaymentRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.UpdatePaymentRequest"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdatePaymentResponse$inboundSchema),
    M.json(400, models.UpdatePaymentResponse$inboundSchema),
    M.json(500, models.UpdatePaymentResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
