...
export function subscriptionsRemoveSubscriptionAddon(
  ...
  Result<
    operations.RemoveSubscriptionAddonResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) => shared.DtoRemoveAddonRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.RemoveSubscriptionAddonResponse$inboundSchema),
    M.json(400, operations.RemoveSubscriptionAddonResponse$inboundSchema),
    M.json(500, operations.RemoveSubscriptionAddonResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
