...
> {
  const parsed = safeParse(
    request,
    (value) => shared.DtoCreateAPIKeyRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateApiKeyResponse$inboundSchema),
    M.json(400, operations.CreateApiKeyResponse$inboundSchema),
    M.json(500, operations.CreateApiKeyResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
