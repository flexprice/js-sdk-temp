...
export function integrationsCreateOrUpdateIntegration(
  ...
  Result<
    operations.CreateOrUpdateIntegrationResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.CreateOrUpdateIntegrationRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.CreateIntegrationRequest"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateOrUpdateIntegrationResponse$inboundSchema),
    M.json(400, operations.CreateOrUpdateIntegrationResponse$inboundSchema),
    M.json(500, operations.CreateOrUpdateIntegrationResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
