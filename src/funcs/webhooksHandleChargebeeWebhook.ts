...
export function webhooksHandleChargebeeWebhook(
  ...
  Result<
    operations.HandleChargebeeWebhookResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.HandleChargebeeWebhookRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/webhooks/chargebee/{tenant_id}/{environment_id}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.HandleChargebeeWebhookResponse$inboundSchema),
    M.json(400, operations.HandleChargebeeWebhookResponse$inboundSchema),
    M.json(401, operations.HandleChargebeeWebhookResponse$inboundSchema),
    M.json(500, operations.HandleChargebeeWebhookResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
