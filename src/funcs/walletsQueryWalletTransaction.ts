...
export function walletsQueryWalletTransaction(
  ...
  Result<
    operations.QueryWalletTransactionResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) => shared.TypesWalletTransactionFilter$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.QueryWalletTransactionResponse$inboundSchema),
    M.json(400, operations.QueryWalletTransactionResponse$inboundSchema),
    M.json(500, operations.QueryWalletTransactionResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
