...
> {
  const parsed = safeParse(
    request,
    (value) => models.UpdateInvoiceRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.UpdateInvoiceRequest"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.UpdateInvoiceResponse$inboundSchema),
    M.json([400, 404], models.UpdateInvoiceResponse$inboundSchema),
    M.json(500, models.UpdateInvoiceResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
