...
> {
  const parsed = safeParse(
    request,
    (value) => models.CancelSubscriptionRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload["dto.CancelSubscriptionRequest"], {
    explode: true,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.CancelSubscriptionResponse$inboundSchema),
    M.json(400, models.CancelSubscriptionResponse$inboundSchema),
    M.json(500, models.CancelSubscriptionResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
