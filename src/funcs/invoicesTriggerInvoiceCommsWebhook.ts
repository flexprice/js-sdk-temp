...
export function invoicesTriggerInvoiceCommsWebhook(
  ...
  Result<
    operations.TriggerInvoiceCommsWebhookResponse,
    | FlexPriceError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.TriggerInvoiceCommsWebhookRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.TriggerInvoiceCommsWebhookResponse$inboundSchema),
    M.json(
      [400, 404],
      operations.TriggerInvoiceCommsWebhookResponse$inboundSchema,
    ),
    M.json(500, operations.TriggerInvoiceCommsWebhookResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
